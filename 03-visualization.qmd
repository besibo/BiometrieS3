...
Quand on a bien compris ce principe, on peut quasiment tout faire, les réponses aux questions qu'on se posent se trouvant presque toujours dans les fichiers d'aide des fonctions.
...
4. Installez et chargez en mémoire le package `nycflight13`
...
- La variable `month` est codée sous forme numérique dans le tableau de données. Il faudra la transformer en factor avec la fonction `factor(month)` au moment de l'associer à un axe du graphique.
