...
principaux de ces séances de TP :
...
À l'issue de ces TP, vous devriez donc être suffisamment à l'aise avec
le logiciel \texttt{RStudio} pour y importer des données issues de
...
\begin{tcolorbox}[enhanced jigsaw, coltitle=black, toprule=.15mm, opacityback=0, breakable, colback=white, opacitybacktitle=0.6, colframe=quarto-callout-important-color-frame, leftrule=.75mm, rightrule=.15mm, titlerule=0mm, bottomtitle=1mm, toptitle=1mm, arc=.35mm, title=\textcolor{quarto-callout-important-color}{\faExclamation}\hspace{0.5em}{Important}, colbacktitle=quarto-callout-important-color!10!white, bottomrule=.15mm, left=2mm]
...
Les travaux pratiques de biométrie auront tous lieu en MSI-217, entre le
7 septembre et le 20 novembre 2026. Chaque groupe aura 5 séances de TP
de 90 minutes selon le calendrier suivant :
...
Au total, chaque groupe aura donc 7h30 de TP. C'est très peu pour
atteindre les objectifs fixés et il y aura donc évidemment du travail
personnel à fournir en dehors de ces séances. \textbf{Pour chaque TP}
prévu à l'emploi du temps, j'estime qu'\textbf{environ trois heures de
travail personnel sont nécessaires}, soit 15 heures de travail personnel
en plus des 7h30 prévues à votre emploi du temps. Selon votre niveau
d'aisance, certains aurons besoin de plus de temps, d'autres d'un peu
moins, mais il est certain que vous devrez travailler en dehors des
heures prévues à votre emploi du temps pour atteindre les objectifs
fixés. \textbf{Attention donc} : pensez bien à prévoir du temps dans vos
plannings \textbf{en dehors des heures prévues dans vos EDT}, car le
travail personnel est essentiel pour progresser dans cette matière.
J'insiste sur l'importance de faire l'effort dès maintenant : vous allez
en effet avoir des enseignements qui reposent sur l'utilisation de ces
logiciels à chaque semestre de la licence, du S3 au S6. C'est maintenant
qu'il faut acquérir des automatismes pour ne pas être perdu plus tard.

\begin{tcolorbox}[enhanced jigsaw, coltitle=black, toprule=.15mm, opacityback=0, breakable, colback=white, opacitybacktitle=0.6, colframe=quarto-callout-warning-color-frame, leftrule=.75mm, rightrule=.15mm, titlerule=0mm, bottomtitle=1mm, toptitle=1mm, arc=.35mm, title=\textcolor{quarto-callout-warning-color}{\faExclamationTriangle}\hspace{0.5em}{Travaillez régulièrement !}, colbacktitle=quarto-callout-warning-color!10!white, bottomrule=.15mm, left=2mm]
...
vos propres ordinateurs, autant prendre dès aujourd'hui de bonnes
habitudes en installant les logiciels dont vous aurez besoin tout au
long de votre licence. Si vous ne possédez pas d'ordinateur,
manifestez-vous rapidement auprès de moi, car des solutions existent
(prêt par l'université, travail sur tablette via
...
qu'en dehors des séances de TP \textbf{où votre présence est
obligatoire}, vous devriez être en mesure d'avancer en autonomie lors de
vos séances de travail personnel.

\end{tcolorbox}

Si vous travaillez sur les ordinateurs de l'université, pensez bien à
vous connecter à la machine virtuelle \texttt{bio}, sur laquelle les
logiciels utiles sont déjà disponibles.
...
Pour apprendre à utiliser des logiciels comme \texttt{R} et
\texttt{RStudio}, il faut faire les choses soi-même, ne pas avoir peur
des messages d'erreurs (il faut d'ailleurs apprendre à les déchiffrer
pour comprendre d'où viennent les problèmes), essayer maintes fois, se
tromper beaucoup, recommencer, et surtout, ne pas se décourager.
J'utilise ce logiciel presque quotidiennement depuis 20 ans et à chaque
session de travail, je rencontre des messages d'erreur. Avec
...
laquelle les chapitres de ce livre doivent être lus attentivement et
\textbf{dans l'ordre}, et les exercices d'application faits au fur et à
mesure de la lecture. Une lecture passive et ``en diagonale'' ne sera
pas suffisante. Il vous faudra taper du code pour reproduire les
exemples et faire les exercices proposés.

Idéalement, voilà les étapes que vous devriez avoir franchies à chaque
séance :
...
\item
  ...
  packages et des scripts. Cette séance doit aussi vous permettre de
  découvrir les objets les plus courants que nous utiliserons lors des
  sessions de travail sous \texttt{R} : les scalaires, les vecteurs, les
  facteurs et les tableaux de données. Avant votre deuxième séance, vous
  devrez avoir terminé le premier chapitre de ce livre, vous devrez être
...
\item
  ...
  vous devrez avoir terminé la section Section~\ref{sec-exo-4}. À ce
  stade, vous devriez donc avoir compris comment charger en mémoire les
  jeux de données disponibles dans un package, vous devriez connaitre la
  syntaxe de base permettant de faire toutes sortes de graphiques avec
  \texttt{ggplot2} avec \textbf{une unique variable numérique}. Vous
  devriez donc notamment connaitre plusieurs façons de représenter la
  distribution d'une variable. À terme, vous devrez être capables de
  choisir des graphiques appropriés selon le nombre et la nature des
  variables dont vous disposez, mais à ce stade, on ne demande rien de
  très complexe. Vous devriez toutefois être capable de créer des
  histogrammes (entre autres), de les décrire et de les interpréter.
\item
  La troisième séance est consacrée à la découverte de graphiques
  permettant de visualiser \textbf{une unique variable catégorielle},
  ainsi que les \textbf{relations entre plusieurs variables} : 2, 3 ou
  plus. Avant la quatrième séance, vous devrez avoir terminé la section
  Section~\ref{sec-simpson}. Vous devriez donc être capable de faire,
  outre les graphiques de la semaine précédente, des nuages de points,
  des stripcharts, des graphiques en lignes et des boîtes à moustaches
  (ou boxplots). Vous devriez aussi être capable de distinguer deux
  types de représentations graphiques assez proches dans leur forme,
  mais fondamentalement différentes dans leur utilisation et leur
  interprétation : les histogrammes et les diagrammes bâtons : dans
  quelle situation utilise-t-on plutôt l'un ou l'autre ? Quel type de
  variable utiliser pour produire un histogramme ou un diagramme bâtons
  ? À quoi correspond l'axe des ordonnées de ces graphiques et comment
  est-il obtenu ? Enfin, vous devriez également être capable de faire
  des sous-graphiques par catégories (\texttt{facet}). Il s'agit là
  d'une possibilité extrêmement puissante pour ``découper'' vos données
  et visualiser les relations entre plusieurs variables.
\item
  Lors de la quatrième séance, vous apprendrez à améliorer la qualité de
  vos graphiques en vue de leur intégration dans un compte-rendu, un
  rapport ou une présentation. Vous verrez comment modifier le thème des
  graphiques, comment ajouter des légendes, et comment modifier les
  échelles choisies automatiquement pour vous par \texttt{ggplot2}. Vous
  devrez aussi commencer le chapitre Chapitre~\ref{sec-wrangling}, et en
  particulier, vous devrez avoir terminé le chapitre
  Section~\ref{sec-exo-10} avant la cinquième et dernière séance de TP.
  Dans ce début de dernier chapitre, vous apprendrez notamment comment
  importer des jeux de données issus de tableurs ou de fichiers textes.
  Jusqu'ici, tous les exemples traités portent sur des jeux de données
  déjà disponibles dans \texttt{R}. Vous apprendrez donc ici à importer
  vos propres jeux de données, et à les mettre en forme pour pouvoir les
  utiliser dans \texttt{R}.
\item
  La cinquième et dernière séance est consacrée à la manipulation des
  tableaux de données. À l'issue de cette séance, vous devrez avoir
  atteint la fin du document, et vous devrez donc être capable de
  sélectionner des colonnes dans un tableau, de filtrer des lignes, et
  de créer de nouvelles variables. Vous devrez aussi avoir compris
  comment utiliser le \texttt{pipe}. Vous devrez être capables
...
\begin{tcolorbox}[enhanced jigsaw, coltitle=black, toprule=.15mm, opacityback=0, breakable, colback=white, opacitybacktitle=0.6, colframe=quarto-callout-warning-color-frame, leftrule=.75mm, rightrule=.15mm, titlerule=0mm, bottomtitle=1mm, toptitle=1mm, arc=.35mm, title=\textcolor{quarto-callout-warning-color}{\faExclamationTriangle}\hspace{0.5em}{Attention au retard}, colbacktitle=quarto-callout-warning-color!10!white, bottomrule=.15mm, left=2mm]
...
pas de retard. Vous notez qu'il y a souvent 2 semaines ou plus entre vos
séances de TP. Il est essentiel de travailler à ces contenus chaque
semaine pour ne pas tout oublier entre deux séances\ldots{}
...
faire beaucoup d'erreurs ! tout récemment, la plupart de nos étudiants
de M1 ont utilisé l'IA pour calculer un indice statistique un peu
exotique. Presque aucun ne s'est rendu compte que le code produit par
l'IA ne faisait pas ce qui était demandé. Par conséquent, toute une
...
demander des améliorations précises, c'est très différent de ne produire
aucun code et de demander à l'IA de tout faire. Dans un cas, on fait
travailler son cerveau, et l'IA joue le rôle d'un partenaire qui fournit
des suggestions souvent pertinentes. Dans l'autre, le cerveau reste au
repos, on n'apprend rien, et donc, on perd son temps\ldots{}

Vous devez donc faire preuve d'esprit critique vis-à-vis de ces outils,
qui peuvent être très utiles, mais qui demandent la plus grande
vigilance et qui se trompent régulièrement. Or, pour faire preuve
d'esprit critique, vous devez être en mesure de comprendre le code
fourni par ces IA. Si ça n'est pas le cas, reprenez les explications de
ce livre en ligne : tout ce dont vous avez besoin pour réussir s'y
trouve noir sur blanc.
...
ces outils, mais à la double condition (i) que ça soit fait à bon
escient (assurez-vous que ce qui est produit par l'IA est pertinent, que
ça a du sens, et que ça reste rare), et (ii), que vous indiquiez
clairement dans vos scripts quelles lignes ont été produites à l'aide
d'une IA (et laquelle).
...
trois groupes de participants lors d'exercices de productions écrites
...
ont le plus appris, car ils ont dû s'investir pleinement dans
l'exercice. En particulier, les auteurs soulignent ceci :

\begin{quote}
Les utilisateurs d'un LLM ont eu du mal à citer correctement leurs
...
vos apprentissages. À vous de faire preuve de maturité et de
discernement pour en tirer le meilleur !
...
récemment apprises et afin que vous soyez en mesure d'auto-évaluer votre
...
vous puissiez me poser des questions lors des TP ou par mail. Tout ce
que nous voyons en TP devra être acquis à la fin des TP. Les TP de
biométrie du semestre 4 (et des deux semestres de L3) supposent en effet
...
\pandocbounded{\includegraphics[keepaspectratio]{images/Grille.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{images/cc_licence.png}}
\end{center}
}
...
commencer par le Chapitre~\ref{sec-dataset}, qui présente un jeu de
données que nous explorerons en détail un peu plus tard. Lisez donc
attentivement ce chapitre et faites bien tous les exercices demandés.

Ce chapitre est en grande partie basé sur les 2 ressources suivantes que
...
\begin{tcolorbox}[enhanced jigsaw, coltitle=black, toprule=.15mm, opacityback=0, breakable, colback=white, opacitybacktitle=0.6, colframe=quarto-callout-warning-color-frame, leftrule=.75mm, rightrule=.15mm, titlerule=0mm, bottomtitle=1mm, toptitle=1mm, arc=.35mm, title=\textcolor{quarto-callout-warning-color}{\faExclamationTriangle}\hspace{0.5em}{Avertissement}, colbacktitle=quarto-callout-warning-color!10!white, bottomrule=.15mm, left=2mm]
...
\item
  ...
  ``\texttt{R-4.4.1.pkg}'' si vous êtes sous Mac avec processeur Intel,
  ou sur \texttt{R-4.4.1-arm64.pkg} si vous êtes sous Mac avec
...
\tightlist
\item
  \href{https://posit.co/download/rstudio-desktop/}{Téléchargez et
  installez \texttt{RStudio}}
\end{enumerate}
...
\item
  ...
  \pandocbounded{\includegraphics[keepaspectratio]{images/Rlogo.png}}
\item
  \texttt{RStudio}, ouvrir cela :
  \pandocbounded{\includegraphics[keepaspectratio]{images/RStudio-Ball.png}}
\end{itemize}
...
\section{Comment exécuter du code ?}\label{sec-code}
...
\begin{tcolorbox}[enhanced jigsaw, coltitle=black, toprule=.15mm, opacityback=0, breakable, colback=white, opacitybacktitle=0.6, colframe=quarto-callout-warning-color-frame, leftrule=.75mm, rightrule=.15mm, titlerule=0mm, bottomtitle=1mm, toptitle=1mm, arc=.35mm, title=\textcolor{quarto-callout-warning-color}{\faExclamationTriangle}\hspace{0.5em}{Les risques du ``copier-coller'' {\faIcon{clipboard}}}, colbacktitle=quarto-callout-warning-color!10!white, bottomrule=.15mm, left=2mm]
...
\begin{tcolorbox}[enhanced jigsaw, coltitle=black, toprule=.15mm, opacityback=0, breakable, colback=white, opacitybacktitle=0.6, colframe=quarto-callout-important-color-frame, leftrule=.75mm, rightrule=.15mm, titlerule=0mm, bottomtitle=1mm, toptitle=1mm, arc=.35mm, title=\textcolor{quarto-callout-important-color}{\faExclamation}\hspace{0.5em}{Définition importante !}, colbacktitle=quarto-callout-important-color!10!white, bottomrule=.15mm, left=2mm]
...
\begin{tcolorbox}[enhanced jigsaw, coltitle=black, toprule=.15mm, opacityback=0, breakable, colback=white, opacitybacktitle=0.6, colframe=quarto-callout-important-color-frame, leftrule=.75mm, rightrule=.15mm, titlerule=0mm, bottomtitle=1mm, toptitle=1mm, arc=.35mm, title=\textcolor{quarto-callout-important-color}{\faExclamation}\hspace{0.5em}{Où sauvegarder vos scripts ? {\faIcon{save}}}, colbacktitle=quarto-callout-important-color!10!white, bottomrule=.15mm, left=2mm]
...
\pandocbounded{\includegraphics[keepaspectratio]{01-R-basics_files/figure-pdf/unnamed-chunk-3-1.pdf}}
...
\begin{center}
\includegraphics[width=0.6\linewidth,height=\textheight,keepaspectratio]{images/Rproj1.png}
\end{center}
...
\begin{center}
\includegraphics[width=0.6\linewidth,height=\textheight,keepaspectratio]{images/Rproj2.png}
\end{center}
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{images/Avant.png}}
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{images/Après.png}}
...
\begin{center}
\includegraphics[width=0.2\linewidth,height=\textheight,keepaspectratio]{images/Rproj3.png}
\end{center}
...
\begin{tcolorbox}[enhanced jigsaw, coltitle=black, toprule=.15mm, opacityback=0, breakable, colback=white, opacitybacktitle=0.6, colframe=quarto-callout-important-color-frame, leftrule=.75mm, rightrule=.15mm, titlerule=0mm, bottomtitle=1mm, toptitle=1mm, arc=.35mm, title=\textcolor{quarto-callout-important-color}{\faExclamation}\hspace{0.5em}{Pour résumer\ldots{} \faIcon{laptop-code}}, colbacktitle=quarto-callout-important-color!10!white, bottomrule=.15mm, left=2mm]
...
\section{Objets et concepts de base}\label{sec-objects}
...
découverte des premiers éléments de syntaxe du langage \texttt{R}. Pour
la suite de ce chapitre, vous devriez travailler dans un Rproject comme
expliqué plus haut, et avoir créé un script vierge dans lequel vous
taperez les commandes \texttt{R} que je vous demande d'exécuter, et
ajouterez les commentaires qui vous semblent utiles.

Dans les 3 sections ci-dessous, nous allons commencer par travailler
avec un jeu de données de taille suffisamment modeste pour pouvoir le
saisir à la main dans \texttt{RStudio}. Nous utiliserons une petite
partie d'un jeu de données de Chris Elphick de l'université du
Connecticut qui contient en tout 7 mesures différentes sur plus de 1100
individus du Bruant à queue aigüe, \emph{Ammodramus caudacutus}, un
passereau de la famille des Passerelidae classé comme ``Vulnérable'' par
l'IUCN. Nous n'utiliserons ici que 4 variables biométriques et une
variable catégorielle mesurées chez 8 individus :

\begin{longtable}[]{@{}rrrrl@{}}

\caption{\label{tbl-bruant}Mesures biométriques de 8 individus du Bruant
à queue aigüe (\emph{Ammodramus caudacutus}). Les variables
correspondent à la longueur des ailes, la longueur du tarse, la longueur
de la tête, la masse et le sexe des individus.}
...
\subsection{Les scalaires}\label{sec-scalaires}

La méthode la plus simple, mais aussi la plus laborieuse, pour entrer
ces données dans \texttt{RStudio} est de les saisir sous forme de
scalaires, c'est-à-dire des variables ne contenant qu'une valeur. Ainsi,
pour les 5 premières observation de longueur d'ailes, nous pourrions
taper :

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{a }\OtherTok{\textless{}{-}} \DecValTok{59}
\NormalTok{b }\OtherTok{\textless{}{-}} \DecValTok{55}
\NormalTok{c }\OtherTok{\textless{}{-}} \FloatTok{53.5}
\NormalTok{d }\OtherTok{\textless{}{-}} \DecValTok{55}
\NormalTok{e }\OtherTok{\textless{}{-}} \FloatTok{52.5}
\end{Highlighting}
\end{Shaded}

Si vous tapez ceci dans votre script et que vous envoyez ces lignes dans
la console, vous ne verrez rien de particulier se passer. Pourtant, 5
nouveaux objets ont été créés en mémoire : leur nom doit apparaître dans
l'onglet ``Environment'' du panneau supérieur droit de l'interface de
\texttt{RStudio}. Pour vérifier que ces objets existent et qu'ils
contiennent les bonnes valeurs, il suffit de taper leur nom dans votre
script et de l'envoyer dans la console :
...
Nous avons donc bien réussi à créer 5 objets, grâce à
l'\textbf{opérateur d'assignation} \texttt{\textless{}-} qui permet de
stocker une valeur dans un objet. Cet opérateur permet donc de créer une
variable (ou objet) dont on indique le nom à gauche de la flèche et d'y
stocker la ou les valeurs indiquées à droite de la flèche. En tapant le
nom de l'objet dans la console, nous demandons à \texttt{R} de nous
afficher la valeur qu'il contient.

Le problème avec cette approche c'est que si nous avons une grande
quantité de données, on va vite tomber à cours de caractères. En outre,
les noms a, b, c, etc, ne sont pas très utiles pour nous rappeler
l'information que contiennent Le problème avec cette approche c'est que
si nous avons une grande quantité de données, on va vite tomber à cours
de caractères. En outre, les noms a, b, c, etc, ne sont pas très utiles
pour nous rappeler l'information que contiennent les variables. On
pourrait du coup utiliser les notations suivantes :

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{aile1 }\OtherTok{\textless{}{-}} \DecValTok{59}
\NormalTok{aile2 }\OtherTok{\textless{}{-}} \DecValTok{55}
\NormalTok{aile3 }\OtherTok{\textless{}{-}} \FloatTok{53.5}
\NormalTok{aile4 }\OtherTok{\textless{}{-}} \DecValTok{55}
\NormalTok{aile5 }\OtherTok{\textless{}{-}} \FloatTok{52.5}
\end{Highlighting}
\end{Shaded}

Cette approche est un peu meilleure que la précédente, mais elle n'est
pas idéale non plus. Imaginez que nous devions saisir les 1100 lignes du
jeu de données complet ! En outre, si nous voulons faire des calculs sur
toutes les longueurs d'ailes, nous allons devoir taper le nom de chaque
variable. Par exemple, pour calculer la moyenne des longueurs d'ailes
des 5 premiers individus, nous devrions taper :

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{(aile1 }\SpecialCharTok{+}\NormalTok{ aile2 }\SpecialCharTok{+}\NormalTok{ aile3 }\SpecialCharTok{+}\NormalTok{ aile4 }\SpecialCharTok{+}\NormalTok{ aile5) }\SpecialCharTok{/} \DecValTok{5}
\end{Highlighting}
\end{Shaded}
...
On imagine bien qu'avec un grand nombre d'observations (par exemple, les
1100 lignes du tableau de données original de Chris Elphick), cette
approche n'est pas tenable.

Avant de voir quelles méthodes plus efficaces nous pourrions utiliser
pour saisir les données, voyons ce que nous pouvons faire de ces
variables. Une fois que nous avons défini une variable et que nous lui
avons attribué une valeur, on peut s'en servir pour effectuer toutes
sortes de calculs. Ainsi, les commandes suivantes sont toutes valides :

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{sqrt}\NormalTok{(aile1)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\DecValTok{2} \SpecialCharTok{*}\NormalTok{ aile1}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{aile1 }\SpecialCharTok{+}\NormalTok{ aile2}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{aile1 }\SpecialCharTok{+}\NormalTok{ aile2 }\SpecialCharTok{+}\NormalTok{ aile3 }\SpecialCharTok{+}\NormalTok{ aile4 }\SpecialCharTok{+}\NormalTok{ aile5}
\end{Highlighting}
\end{Shaded}
...
La fonction \texttt{sqrt()} permet de calculer la racine carrée d'un
nombre (c'est l'abréviation de ``square root''). Nous verrons plus en
détail ce que sont les fonctions un peu plus loin dans ce chapitre. Pour
l'instant, retenez simplement que les fonctions prennent en entrée une
ou plusieurs valeurs, et produisent une ou plusieurs valeurs en sortie.

Bien que \texttt{R} fasse les calculs, il ne garde pas les résultats en
mémoire. Il est certainement mieux de définir de nouvelles variables si
nous souhaitons réutiliser les résultats des calculs précédents :

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{rac\_aile1 }\OtherTok{\textless{}{-}} \FunctionTok{sqrt}\NormalTok{(aile1)}
\NormalTok{prod\_a1 }\OtherTok{\textless{}{-}} \DecValTok{2} \SpecialCharTok{*}\NormalTok{ aile1}
\NormalTok{somme\_12 }\OtherTok{\textless{}{-}}\NormalTok{aile1 }\SpecialCharTok{+}\NormalTok{ aile2}
\NormalTok{somme\_12345 }\OtherTok{\textless{}{-}}\NormalTok{ aile1 }\SpecialCharTok{+}\NormalTok{ aile2 }\SpecialCharTok{+}\NormalTok{ aile3 }\SpecialCharTok{+}\NormalTok{ aile4 }\SpecialCharTok{+}\NormalTok{ aile5}
\NormalTok{moy }\OtherTok{\textless{}{-}}\NormalTok{ (aile1 }\SpecialCharTok{+}\NormalTok{ aile2 }\SpecialCharTok{+}\NormalTok{ aile3 }\SpecialCharTok{+}\NormalTok{ aile4 }\SpecialCharTok{+}\NormalTok{ aile5) }\SpecialCharTok{/} \DecValTok{5}
\end{Highlighting}
\end{Shaded}

Ces 5 lignes vous montrent que l'on peut utiliser n'importe quel nom
pour les variables. Notez que le tiret bas fait ici partie du nom des
variables. Je vous conseille fortement d'utiliser des noms courts et
parlants, c'est-à-dire des noms qui vous permettront de vous souvenir ce
que contient la variable. Par exemple, la variable
\texttt{rac\_.\_aile1} contient la racine carrée de la longueur de
l'aile pour l'individu 1. Vous devrez donc faire preuve d'imagination et
de creativité : trouver des noms courts et parlants n'est pas toujours
évident ! Il vous faudra en outre éviter d'utiliser des noms contenant
des caractères speciaux tels que \texttt{\$}, \texttt{\%}, \texttt{\&},
\texttt{*}, \texttt{\^{}}, \texttt{+}, \texttt{-}, \texttt{\#},
\texttt{()}, \texttt{{[}{]}}, \texttt{\{\}}, \texttt{\textless{}},
\texttt{\textgreater{}}, \texttt{?}, \texttt{!}, \texttt{@},
\texttt{\textasciitilde{}}, etc. Ces caractères ont en effet un sens
particulier pour \texttt{R} et conduiraient inévitablement à des
erreurs. Certains mots sont également réservés par \texttt{R} et ne
peuvent pas être utilisés comme noms de variables. C'est le cas par
exemple des mots \texttt{if}, \texttt{else}, \texttt{repeat},
\texttt{while}, \texttt{function}, \texttt{for}, \texttt{in},
\texttt{next}, \texttt{break}, \texttt{TRUE}, \texttt{FALSE},
\texttt{NULL}, \texttt{Inf}, \texttt{NaN}, \texttt{NA}, etc. Il est donc
tr`´s fortement déconseillé d'utiliser ces noms pour vos variables.

\subsection{Les vecteurs}\label{sec-vectors}

\subsubsection{Création et manipulations
simples}\label{cruxe9ation-et-manipulations-simples}

Avec la méthode des scalaires décrite ci-dessous, pour 8 individus et 5
variables, nous aurions besoin de 40 noms d'objets différents. Une
meilleure façon de procéder est de stocker plusieurs valeurs dans un
même objet. Pour cela, nous avons besoin de la fonction \texttt{c()},
qui permet de \texttt{c}ombiner ou \texttt{c}oncaténer plusieurs valeurs
au sein dun unique \textbf{vecteur} :

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{ailes }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}\DecValTok{59}\NormalTok{, }\DecValTok{55}\NormalTok{, }\FloatTok{53.5}\NormalTok{, }\DecValTok{55}\NormalTok{, }\FloatTok{52.5}\NormalTok{, }\FloatTok{57.5}\NormalTok{, }\DecValTok{53}\NormalTok{, }\DecValTok{55}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

Il est important ici de bien utiliser les parenthèses et non les
crochets ou les accolades qui ont une autre signification. D'une façon
générale, dans \texttt{R}, un nom de fonction est toujours suivi de
parenthèses. C'est par exemple le cas des fonctions \texttt{log()},
\texttt{log10()}, \texttt{getwd()} que nous connaissons déjà.

Comme précédemment, écrire la commande ci-dessus dans votre script et
l'envoyer dans la console ne fait que créer l'objet \texttt{ailes}. Pour
afficher son contenu, envoyez simplement le nom de l'objet dans la
console :
...
On constate que l'objet \texttt{ailes} contient bien les 8 valeurs que
nous avons saisies. Nous pouvons également vérifier le type de cet objet
avec la fonction \texttt{class()} :

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{class}\NormalTok{(ailes)}
\end{Highlighting}
\end{Shaded}
...
Il s'agit bien ici d'un vecteur qui contient des données numériques. La
fonction \texttt{c()} a créé un unique vecteur contientant 8 éléments
qui sont tous du même type. On dit qu'il est ``de longueur 8''. Pour
déterminer la longueur d'un vecteur quelconque, il suffit de taper :

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{length}\NormalTok{(ailes)}
\end{Highlighting}
\end{Shaded}
...
\subsubsection{L'indexation}\label{lindexation}

Le terme ``indexation'' désigne l'ensemble des méthodes qui permettent
d'accéder à des éléments spécifiques au sein d'un objets qui en contient
plusieurs. Les vecteurs sont les objets les plus simples qui contiennent
plusieurs éléments. Nous allons donc ici découvrir comment accéder à des
éléments specifiques au sein des vecteurs.

Pour cela, la méthode historique qui fonctionnera toujours consiste à
utiliser les crochets. Par exemple, pour voir uniquement la première
valeur du vecteur \texttt{ailes}, tapez :

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{ailes[}\DecValTok{1}\NormalTok{]}
\end{Highlighting}
\end{Shaded}
...
Notez bien que l'indice 1 correspond au premier élément du vecteur. Dans
d'autres langages de programmation (comme en Python), le premier élément
d'un vecteur porte l'indice 0.

Nous ne sommes pas limités à aficher une unique valeur à la fois. Ainsi,
pour afficher les éléments situés à la première, troisième et quatrième
position de \texttt{ailes}, on tape :

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{ailes[}\FunctionTok{c}\NormalTok{(}\DecValTok{1}\NormalTok{, }\DecValTok{4}\NormalTok{, }\DecValTok{3}\NormalTok{)]}
\end{Highlighting}
\end{Shaded}
...
Il est également possible de fournir un ou des indices négatifs entre
crochets afin de retirer des valeurs. Pour afficher toutes les valeurs
de \texttt{ailes} sauf la deuxième, on tape :

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{ailes[}\SpecialCharTok{{-}}\DecValTok{2}\NormalTok{]}
\end{Highlighting}
\end{Shaded}
...
Ces méthodes qui permettent d'accéder à des éléments spécifiques au sein
d'un vecteur relèvent toutes de ce que l'on appelle l'\textbf{indexation
par position}. Elles supposent en effet de connaître à l'avance la
position des éléments que l'on souhaite récupérer à l'intérieur de
l'objet manipulé. Malheureusement, ça n'est pas toujours le cas. On peut
par exemple avoir besoin d'aficher uniquement les longueur d'ailes qui
dépassent une valeur donnée. Si le vecteur contient de nombreuses
valeurs, il est impossible de taper manuellement les indices pertinents.
On utilise alors plutôt l'\textbf{indexation par comparaison}. Par
exemple, pour récupérer toutes les ailes dont les longueurs sont
supérieures à 56 mm, on tape ceci :

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{ailes[ailes }\SpecialCharTok{\textgreater{}} \DecValTok{56}\NormalTok{]}
\end{Highlighting}
\end{Shaded}
...
On constate qu'il n'y en a que 2. Vous verrez dans le
Chapitre~\ref{sec-wrangling} plusieurs méthodes modernes permettant de
faire la même chose de façon plus intuitive. À ce stade, vous pouvez
retenir que :

\begin{itemize}
\tightlist
\item
  l'objet dont on fournit le nom à gauche des crochets est l'objet dans
  lequel on veut récupérer des valeurs
\item
  l'objet dont le nom apparaît à l'intérieur des crochets est l'objet
  sur lequel on fait porter une condition.
\end{itemize}

Ici, il s'agit du même objet, mais ça n'est pas obligatoire. Si on
disposait d'un vecteur \texttt{masse} contenant les masses des 8 oiseaux
du tableau étudié, on pourrait demander à \texttt{R} de nous afficher
les masses des oiseaux dont la longueur des ailes dépasse 56 mm :

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Création du vecteur des masses}
\NormalTok{masse }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}\FloatTok{9.5}\NormalTok{, }\FloatTok{13.8}\NormalTok{, }\FloatTok{14.8}\NormalTok{, }\FloatTok{15.2}\NormalTok{, }\FloatTok{15.5}\NormalTok{, }\FloatTok{15.6}\NormalTok{, }\FloatTok{15.6}\NormalTok{, }\FloatTok{15.7}\NormalTok{)}

\CommentTok{\# Affichage des masses des oiseaux dont la longueur des ailes dépasse 56 mm}
\NormalTok{masse[ailes }\SpecialCharTok{\textgreater{}} \DecValTok{56}\NormalTok{]}
\end{Highlighting}
\end{Shaded}
...
Ici, on crée le vecteur \texttt{masse}, puis on va chercher dans ce
vecteur (son nom est à gauche des crochets) les valeurs de masses des
oiseaux, dont la longueur des ailes dépasse 56 mm (cette condition est
indiquée à l'intérieur des crochets). On constate que ces oiseaux sont
également les plus lourds.

Avec ce type de syntaxe simple, il est possible de faire des
comparaisons relativement élaborées et donc d'accéder à des données qui
respecent plusieurs conditions à la fois. Quelques exemples sont fournis
plus bas, dans la Section~\ref{sec-comparaison}.

\subsubsection{Autres fonctions utiles pour les
vecteurs}\label{autres-fonctions-utiles-pour-les-vecteurs}

Si la fonction \texttt{c()} est certainement de très loin celle que vous
utiliserez le plus souvent pour créer des vecteurs quelconques, il
existe d'autres façons de créer des vecteurs. Par exemple, l'operateur
\texttt{:} permet de créer des vecteurs contenant des séquences de
nombres entiers. Par exemple, pour créer un vecteur contenant les
...
Cette notation a le merite d'être très concise, mais elle n'est pas très
flexible. En effet, on ne peut pas modifier le pas entre chaque valeur :
on crée obligatoirement des suites d'entiers, avec un pas de 1. Pour
plus de souplesses, on peut utiliser la fonction \texttt{seq()}. On
fournit un point de départ, un point d'arrivée, et un pas entre chaque
valeur, et la fonction s'occupe du reste. Par exemple, pour créer un
vecteur contenant les nombres allant de 1 à 10 avec un pas de 2 entre
chaque valeur, on tape :
...
Notez qu'en partant de 1, on n'atteint pas 10 avec un pas de 2. La
dernière valeur du vecteur est donc 9. De même, pour créer un vecteur
décroissant, qui va de 100 à 0 de 5 en 5, on tape :

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{seq}\NormalTok{(}\AttributeTok{from =} \DecValTok{100}\NormalTok{, }\AttributeTok{to =} \DecValTok{0}\NormalTok{, }\AttributeTok{by =} \SpecialCharTok{{-}}\DecValTok{5}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
Faites bien attention au signe de l'argument \texttt{by} : il doit être
négatif si la séquence est décroissante.

Un autre syntaxe de la fonction \texttt{seq()} est également possible.
On peut en effet fournir un point de départ, un point d'arrivée et le
nombre de valeurs que l'on souhaite dans le vecteur grâce à l'argument
\texttt{length.out}. Par exemple, pour créer un vecteur contenant 12
valeurs allant de 1 à 100, on peut taper :

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{seq}\NormalTok{(}\AttributeTok{from =} \DecValTok{1}\NormalTok{, }\AttributeTok{to =} \DecValTok{100}\NormalTok{, }\AttributeTok{length.out =} \DecValTok{13}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
 [1]   1.00   9.25  17.50  25.75  34.00  42.25  50.50  58.75  67.00  75.25
[11]  83.50  91.75 100.00
\end{verbatim}

En précisant le nombre de valeurs que l'on souhaite pour le vecteur, on
charge la fonction de calculer automatiquement le pas entre chaque
valeur.

Une autre fonction très utile pour créer des vecteurs est la fonction
\texttt{rep()}, qui permet de répéter une ou plusieurs valeurs un
certain nombre de fois. Par exemple, pour créer un vecteur contenant 10
fois la valeur 5, on tape :

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{rep}\NormalTok{(}\AttributeTok{x =} \DecValTok{5}\NormalTok{, }\AttributeTok{times =} \DecValTok{10}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
Pour créer un vecteur contenant les valeurs 1, 2 et 3, chacune répétée 4
fois, on tape :

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{rep}\NormalTok{(}\AttributeTok{x =} \DecValTok{1}\SpecialCharTok{:}\DecValTok{3}\NormalTok{, }\AttributeTok{times =} \DecValTok{4}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
On peut également créer un vecteur contenant les valeurs 1, 2 et 3, en
répétant chacune de ces valeurs 4 fois avant de passer à la suivante.
Pour cela, on utilise l'argument \texttt{each} :

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{rep}\NormalTok{(}\AttributeTok{x =} \DecValTok{1}\SpecialCharTok{:}\DecValTok{3}\NormalTok{, }\AttributeTok{each =} \DecValTok{4}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
Prises separément, toutes ces methodes sont simples, mais elles
deviennent très puissantes lorsqu'elles sont combinées et vous ne
devriez pas avoir besoin de beaucoup d'autres fonctions que
\texttt{c()}, \texttt{seq()}, \texttt{rep()} et \texttt{:} pour créer
des vecteurs.

\subsubsection{Autres types de vecteurs}\label{autres-types-de-vecteurs}

Nous n'avons jusqu'ici travaillé qu'avec des vecteurs numériques.
Pourtant, \texttt{R} permet de créer des vecteurs contenant d'autres
types de données. Par exemple, pour créer un vecteur contenant des
chaînes de caractères (des mots ou des phrases), on utilise la fonction
\texttt{c()} en mettant les chaînes de caractères entre guillemets
simples ou doubles :

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{animaux }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}\StringTok{"chat"}\NormalTok{, }\StringTok{"chien"}\NormalTok{, }\StringTok{"souris"}\NormalTok{, }\StringTok{"hamster"}\NormalTok{, }\StringTok{"baleine"}\NormalTok{)}
\NormalTok{animaux}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
[1] "chat"    "chien"   "souris"  "hamster" "baleine"
\end{verbatim}

Évidemment, il n'est pas possible de faire des calculs avec de tels
vecteurs, contrairement aux vecteurs numeriques :

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Calcul de la moyenne des longueurs d\textquotesingle{}ailes}
\FunctionTok{mean}\NormalTok{(ailes)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# un calcul de moyenne qui n\textquotesingle{}a pas de sens...}
\FunctionTok{mean}\NormalTok{(animaux)}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
Warning in mean.default(animaux): l'argument n'est ni numérique, ni logique :
renvoi de NA
\end{verbatim}
...
Outre les types numériques et caractères, \texttt{R} permet aussi de
créer des vecteurs logiques, c'est-à-dire des vecteurs ne contenant que
les valeurs \texttt{TRUE} (vrai) ou \texttt{FALSE} (faux). Attention,
\texttt{TRUE} et \texttt{FALSE} doivent être écrits en majuscules et
sans guillemets :

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{vf }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}\ConstantTok{TRUE}\NormalTok{, }\ConstantTok{FALSE}\NormalTok{, }\ConstantTok{TRUE}\NormalTok{, }\ConstantTok{TRUE}\NormalTok{, }\ConstantTok{FALSE}\NormalTok{)}
\NormalTok{vf}
\end{Highlighting}
\end{Shaded}
...
Des variables logiques (vrais/faux) seront souvent présentes dans vos
tableaux de données, par exemple pour indiquer quels individus ont une
taille ou une masse qui dépasse un certain seuil. Grâce au vecteur
\texttt{masse} créé plus tôt, on peut ainsi créer un vecteur logique
indiquant quels individus ont une masse supérieure à 15 g par exemple :

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{masse\_sup\_15 }\OtherTok{\textless{}{-}}\NormalTok{ masse }\SpecialCharTok{\textgreater{}} \DecValTok{15}
\NormalTok{masse\_sup\_15}
\end{Highlighting}
\end{Shaded}
...
Contrairement aux vecteurs de caractères, il est possible de faire des
calculs avec des vecteurs logiques. En effet, dans \texttt{R}, la valeur
\texttt{TRUE} est équivalente à 1 et la valeur \texttt{FALSE} est
équivalente à 0. Ainsi, en calculant la somme d'un vecteur logique, on
obtient le nombre de valeurs \texttt{TRUE} qu'il contient, et en
calculant la moyenne, on obtient la proportion de vrais :

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Nombre de valeurs TRUE dans le vecteur logique masse\_sup\_15}
\FunctionTok{sum}\NormalTok{(masse\_sup\_15)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Proportion de valeurs TRUE dans le vecteur logique masse\_sup\_15}
\FunctionTok{mean}\NormalTok{(masse\_sup\_15)}
\end{Highlighting}
\end{Shaded}
...
Nous avons vu que des vecteurs pouvaient contenir des données
numériques, des chaînes de caractères, des valeurs logiques
(vrais/faux). Mais il existe de nombreux autres types de variables que
vous serez peut-être amenés à rencontrer (par exemple, des dates, des
heures, des latitudes, des longitudes et d'autres données plus
complexes). Pour savoir quel est le type d'un vecteur quelconque, on
peut utiliser la fonction \texttt{class()} :
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{class}\NormalTok{(animaux)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{class}\NormalTok{(vf)}
\end{Highlighting}
\end{Shaded}
...
Les vecteurs sont omniprésents dans \texttt{R} et vous serez amenés à en
créer et à en manipuler très souvent. En particulier, les tableaux de
données sont la plupart du temps une simple juxtaposition de vecteurs de
plusieurs types : chaque colonne est un vecteur (ou un facteur, voir
plus bas). Il est donc important que vous vous familiarisiez avec ces
objets dès maintenant. Nous verrons dans le Chapitre~\ref{sec-wrangling}
plusieurs fonctions très utiles pour manipuler des vecteurs, des
facteurs et des tableaux.

Avant d'aller plus loin, créez les autres vecteurs correspondant aux
autres variables du tableau de données. Vous devriez ainsi créer les
vecteurs \texttt{tarse}, \texttt{tete}, et \texttt{sexe}, en plus des
vecteurs \texttt{ailes} et \texttt{masse} que vous avez déjà créés. Si
vous y parvenez, les commandes suivantes devraient afficher exactement
ces résultats :
...
\begin{verbatim}
[1] "femelle" "mâle"    "mâle"    "mâle"    "femelle" "mâle"    "femelle"
[8] "femelle"
\end{verbatim}

\subsection{Les facteurs}\label{sec-factors}

Dans \texttt{R}, les vecteurs sont des collections d'éléments qui sont
tous du même type. Ils sont très largement utilisés, et servent
notamment à stocker les valeurs de variables numériques comme nous
l'avons vu plus haut. Cependant, dans les sciences de la vie, on
manipule très souvent des variables catégorielles, c'est-à-dire des
variables qui ne sont pas numériques. Par exemple, le sexe des
individus, leur statut de survie (mort/vivant), leur habitat (forêt,
prairie, zone humide, étage de l'estran, \ldots), leur espèce,
\emph{etc.} Dans \texttt{R}, les variables catégorielles sont stockées
sous forme de \textbf{facteurs} (ou \texttt{factor}s en anglais). Pour
créer un facteur, on utilise la fonction \texttt{factor()}.

Pour illustrer cela, nous allons nous concentrer sur la variable
\texttt{sexe} du tableau de données. Nous avons déjà créé un vecteur
\texttt{sexe} contenant les valeurs ``femelle'' et ``mâle'' dans le bon
ordre :
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{sexe }\OtherTok{\textless{}{-}} \FunctionTok{factor}\NormalTok{(sexe)}
\end{Highlighting}
\end{Shaded}

L'objet \texttt{sexe} est maintenant un facteur. Pour vérifier cela, on
peut utiliser la fonction \texttt{class()} :

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{class}\NormalTok{(sexe)}
\end{Highlighting}
\end{Shaded}
...
Lorsqu'on affiche le contenu d'un facteur, \texttt{R} affiche non
seulement le contenu de l'objet (ici, les 8 valeurs de ``femelle'' et
``mâle'' que nous avions saisies initialement dans le vecteur), mais en
plus, il affiche un attribut supplementaire, qui n'existe que por les
facteurs (donc qui n'existe pas pour les vecteurs) nommé
\texttt{Levels}. Les ``niveaux'' ou ``levels'', correspndent aux
\textbf{modalités} (ou catégories possibles) d'une variable
catégorielle.
...
\begin{verbatim}
[1] femelle mâle    mâle    mâle    femelle mâle    femelle femelle
Levels: femelle mâle
\end{verbatim}

Ici, il n'y a donc que 2 niveaux, 2 catégories possibles pour la
variable \texttt{sexe} : ``femelle'' ou ``mâle''.

L'intérêt de travailler avec des facteurs plutôt qu'avec des vecteurs
pour les variables categorielles est que nous aurons la possibilité de
modifier manuellement à notre guise les niveaux d'un facteur. Ainsi, si
on souhaite inverser l'ordre des niveaux du facteur \texttt{sexe}, par
exemple pour que les mâles apparaissent à gauche des femelles sur un
graphique, on tape :

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{sexe }\OtherTok{\textless{}{-}} \FunctionTok{factor}\NormalTok{(sexe, }\AttributeTok{levels =} \FunctionTok{c}\NormalTok{(}\StringTok{"mâle"}\NormalTok{, }\StringTok{"femelle"}\NormalTok{))}
\NormalTok{sexe}
\end{Highlighting}
\end{Shaded}
...
Vous pouvez constater ici que l'ordre des données n'a pas changé (les
femelles sont toujours en position 1, 5, 7 et 8), mais que l'ordre des
niveaux a bien été modifié (le niveau ``mâle'' est maintenant le premier
niveau). Nous verrons dans le Chapitre~\ref{sec-viz} comment cet ordre
des niveaux influence l'affichage des données sur un graphique.

Cette réorganisation manuelle des niveaux d'un facteur est très utile,
car par défaut, \texttt{R} organise les niveaux d'un facteur par ordre
alphabétique. Si on dispose par exemple d'un facteur \texttt{mois}
contenant les noms des mois de l'année (``janvier'', ``février'',
``mars'', \ldots), par défaut, les niveaux seront organisés dans l'ordre
alphabétique (``avril'', ``août'', ``décembre'', ``février'', \ldots),
ce qui n'a pas de sens. Pour faire des graphiques qui ont du sens, et
faire apparaitre les mois dans l'ordre chronologique, il nous faudrait
réorganiser les niveaux manuellement grâce à l'argument \texttt{levels}
de la fonction \texttt{factor()}.

Enfin, outre la fonction \texttt{factor()} qui possède un
\textbf{argument} \texttt{levels}, il existe aussi une \textbf{fonction}
nommée \texttt{levels()}. Cette fonction permet de faire 2 choses. tout
d'abord, elle permet d'afficher les niveaux d'un facteur :

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{levels}\NormalTok{(sexe)}
\end{Highlighting}
\end{Shaded}
...
Notez que les niveaux sont ici présentés dans l'ordre que j'ai imposé
précédemment, et non pas dans l'ordre alphabétique. La fonction
\texttt{levels()} permet par ailleurs de modifier les niveaux d'un
facteur. Par exemple, pour renommer les niveaux du facteur \texttt{sexe}
en \texttt{"Fe"} et \texttt{"Ma"}, on tape ceci :

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{levels}\NormalTok{(sexe) }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}\StringTok{"Ma"}\NormalTok{, }\StringTok{"Fe"}\NormalTok{)}
\NormalTok{sexe}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
[1] Fe Ma Ma Ma Fe Ma Fe Fe
Levels: Ma Fe
\end{verbatim}

\begin{tcolorbox}[enhanced jigsaw, coltitle=black, toprule=.15mm, opacityback=0, breakable, colback=white, opacitybacktitle=0.6, colframe=quarto-callout-important-color-frame, leftrule=.75mm, rightrule=.15mm, titlerule=0mm, bottomtitle=1mm, toptitle=1mm, arc=.35mm, title=\textcolor{quarto-callout-important-color}{\faExclamation}\hspace{0.5em}{Attention à l'ordre des niveaux !}, colbacktitle=quarto-callout-important-color!10!white, bottomrule=.15mm, left=2mm]

Il est essentiel de respecter {l'ordre des niveaux} lorsque l'on renomme
les niveaux d'un facteur.

Ici, le niveau \texttt{"mâle"} était placé avant le niveau
\texttt{"femelle"} dans le facteur \texttt{sexe}. En renommant les
niveaux, j'ai donc dû respecter cet ordre, et taper
\texttt{c("Ma",\ "Fe")} et non pas \texttt{c("Fe",\ "Ma")}. Si j'avais
tapé \texttt{c("Fe",\ "Ma")}, j'aurais involontairement inversé les
niveaux du facteur : les mâles seraient devenus des femelles, et les
femelles des mâles ! Faites donc très attention à l'ordre des niveaux
lorsque vous renommez les niveaux d'un facteur, que ce soit avec la
fonction \texttt{factor()} ou avec la fonction \texttt{levels()}.

\end{tcolorbox}

\subsection{Les data.frames}\label{sec-dataframe}

Un \textbf{data.frame} (ou tableau de données) est une structure de
données rectangulaire, c'est-à-dire un tableau à deux dimensions, qui
peut contenir plusieurs types de variables. Chaque colonne d'un
\texttt{data.frame} est un vecteur (ou un facteur) contenant les valeurs
d'une variable, et chaque ligne correspond aux différentes valeurs
mesurées pour une observation. Les \texttt{data.frame}s sont très
largement utilisés dans \texttt{R}, car ils permettent de stocker des
jeux de données complets, avec plusieurs variables de différents types
(numériques, catégorielles, logiques, \emph{etc.}). Pour créer un
\texttt{data.frame}, on utilise la fonction \texttt{data.frame()}, en
fournissant les différents vecteurs (ou facteurs) que l'on souhaite
inclure dans le tableau de données. Par exemple, pour créer un
\texttt{data.frame} contenant les variables \texttt{ailes},
\texttt{tarse}, \texttt{tete}, \texttt{masse} et \texttt{sexe} que nous
avons créées précédemment, on tape :

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Création d\textquotesingle{}un data.frame}
\NormalTok{Bruant }\OtherTok{\textless{}{-}} \FunctionTok{data.frame}\NormalTok{(}
  \AttributeTok{Ailes =}\NormalTok{ ailes,}
  \AttributeTok{Tarse =}\NormalTok{ tarse,}
  \AttributeTok{Tete =}\NormalTok{ tete,}
  \AttributeTok{Masse =}\NormalTok{ masse,}
  \AttributeTok{Sexe =}\NormalTok{ sexe}
\NormalTok{)}

\CommentTok{\# Affichage de son contenu}
\NormalTok{Bruant}
\end{Highlighting}
\end{Shaded}
...
8  55.0  21.5   NA  15.7   Fe
\end{verbatim}

Notez que pour les nommer les colonnes (à gauche des signes \texttt{=}
dans le code ci-dessus), j'ai choisi des noms avec une majuscule pour
les distinguer des noms de vecteurs et facteurs créés au préalable sans
majuscule. Ça n'est évidemment pas obligatoire et vous pouvez choisir
les noms que vous souhaitez pour les colonnes d'un \texttt{data.frame}.
Je vous conesille toutefois d'éviter simplement les caractères spéciaux
et les mots réservés par \texttt{R} comme expliqué plus haut. Choisissez
des noms courts et parlants pour les colonnes de vos tableaux de données
car vous aurez besoin de les taper souvent pour accéder au contenu de
chaque variable.

\subsubsection{Dimensions et indexation par
position}\label{dimensions-et-indexation-par-position}

Comme pour les vecteurs et les facteurs, il est possible d'accéder à des
éléments spécifiques au sein d'un \texttt{data.frame}. Pour cela, on
peut utiliser les crochets. Cependant, comme un \texttt{data.frame} est
une structure à deux dimensions (lignes et colonnes), il faut cette fois
fournir deux indices entre crochets : le premier indice correspond à la
ligne, et le second indice correspond à la colonne. Par exemple, pour
accéder à la valeur de la première ligne et de la deuxième colonne du
\texttt{data.frame} \texttt{Bruant}, on tape :

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Accéder à la valeur de la première ligne et de la deuxième colonne du data.frame Bruant}
\NormalTok{Bruant[}\DecValTok{1}\NormalTok{, }\DecValTok{2}\NormalTok{]}
\end{Highlighting}
\end{Shaded}
...
On peut également accéder à une ligne entière ou à une colonne entière
en omettant l'un des deux indices. Par exemple, pour accéder à la
première ligne entière du \texttt{data.frame} \texttt{Bruant}, on tape :

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{Bruant[}\DecValTok{1}\NormalTok{, ]}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
  Ailes Tarse Tete Masse Sexe
1    59  22.3 31.2   9.5   Fe
\end{verbatim}

De même, pour accéder à la deuxième colonne entière du
\texttt{data.frame} \texttt{Bruant}, on tape :

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{Bruant[, }\DecValTok{2}\NormalTok{]}
\end{Highlighting}
\end{Shaded}
...
Omettre l'indice des lignes (ou des colonnes) revient donc à demander à
\texttt{R} de nous afficher toutes les lignes (ou toutes les colonnes
respectivement).

On peut aussi demander plusieurs lignes et/ou plusieurs colonnes en
fournissant des indices entre crochets. Par exemple, pour afficher les
lignes 1, 3 et 5 du \texttt{data.frame} \texttt{Bruant}, on tape :

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{Bruant[}\FunctionTok{c}\NormalTok{(}\DecValTok{1}\NormalTok{, }\DecValTok{3}\NormalTok{, }\DecValTok{5}\NormalTok{), ]}
\end{Highlighting}
\end{Shaded}
...
Enfin, il est aussi possible, comme pour les vecteurs, de fournir des
indices négatifs entre crochets afin de retirer des lignes ou des
colonnes. Par exemple, pour afficher toutes les colonnes du
\texttt{data.frame} \texttt{Bruant} sauf la quatrième, on tape :

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{Bruant[, }\SpecialCharTok{{-}}\DecValTok{4}\NormalTok{]}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
  Ailes Tarse Tete Sexe
1  59.0  22.3 31.2   Fe
2  55.0  19.7 30.4   Ma
3  53.5  20.8 30.6   Ma
4  55.0  20.3 30.3   Ma
...
Toutes ces commandes ne fonctionnent que si les indices fournis entre
crochets ont un sens. Par exemple, ce code génère un message d'erreur :

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{Bruant[,}\DecValTok{10}\NormalTok{]}
\end{Highlighting}
\end{Shaded}
...
Ici, je demande à \texttt{R} d'afficher la dixième colonne du
\texttt{data.frame} \texttt{Bruant}, mais ce tableau ne contient que 5
variables. Il n'y a donc pas de dixième colonne à afficher, et
\texttt{R} le fait savoir avec un message d'erreur. Pour travailler
efficacement avec des \texttt{data.frame}, il est donc important de
connaître leurs dimensions. Pour cela, on peut utiliser la fonction
\texttt{dim()}, qui affiche le nombre de lignes et le nombre de colonnes
d'un \texttt{data.frame}, ou les fonctions \texttt{nrow()} et
\texttt{ncol()}, qui affichent respectivement le nombre de lignes et le
nombre de colonnes :

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Dimensions du data.frame Bruant}
\FunctionTok{dim}\NormalTok{(Bruant)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Nombre de lignes du data.frame Bruant}
\FunctionTok{nrow}\NormalTok{(Bruant)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Nombre de colonnes du data.frame Bruant}
\FunctionTok{ncol}\NormalTok{(Bruant)}
\end{Highlighting}
\end{Shaded}
...
On peut aussi utiliser la fonction \texttt{str()} pour afficher la
structure d'un \texttt{data.frame}, c'est-à-dire le nombre de lignes et
de colonnes, ainsi que le type de chaque variable (numérique, facteur,
\emph{etc.}) et les premières valeurs de chaque variable :

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{str}\NormalTok{(Bruant)}
\end{Highlighting}
\end{Shaded}
...
'data.frame':   8 obs. of  5 variables:
 ...
 $ Sexe : Factor w/ 2 levels "Ma","Fe": 2 1 1 1 2 1 2 2
\end{verbatim}

Enfin, on peut aussi regarder dans l'onget ``Environment'' du panneau
supérieur droit de l'interface de \texttt{RStudio} pour voir les
dimensions du \texttt{data.frame} \texttt{Bruant}, ainsi que le type de
chaque variable créée depuis le début de la session de travail.

Il est possible d'accéder aux colonnes d'un \texttt{data.frame} en
utilisant le signe \texttt{\$}. Par exemple, pour accéder à la colonne
\texttt{Masse} du \texttt{data.frame} \texttt{Bruant}, on peut taper :

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{Bruant}\SpecialCharTok{$}\NormalTok{Masse}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{Bruant[, }\StringTok{"Masse"}\NormalTok{]}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\CommentTok{\# ou à }
\NormalTok{Bruant[, }\DecValTok{4}\NormalTok{]}
\end{Highlighting}
\end{Shaded}
...
Vous le voyez, il y a très souvent plusieurs façons de faire la même
chose dans \texttt{R}. C'est souvent une source de confusion pour les
débutants, mais c'est aussi une force du langage : avec l'expérience,
vous apprendrez à choisir la méthode qui vous convient le mieux.

\subsubsection{Indexation par
comparaison}\label{indexation-par-comparaison}

Comme pour les vecteurs, on souhaite souvent accéder à certaines lignes
d'un tableau de données sans connaître à l'avance leur position. Par
exemple, on peut vouloir afficher toutes les lignes du
\texttt{data.frame} \texttt{Bruant} où le sexe est ``Fe'', ou encore,
les lignes pour lesquelles la masse des individus est supérieure à 15 g.
Voilà ce qu' il faudrait taper dans l'un et l'autre cas :

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Afficher les lignes du data.frame Bruant où le sexe est "Fe"}
\NormalTok{Bruant[Bruant}\SpecialCharTok{$}\NormalTok{Sexe }\SpecialCharTok{==} \StringTok{"Fe"}\NormalTok{, ]}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Afficher les lignes du data.frame Bruant où la masse est supérieure à 15 g}
\NormalTok{Bruant[Bruant}\SpecialCharTok{$}\NormalTok{Masse }\SpecialCharTok{\textgreater{}} \DecValTok{15}\NormalTok{, ]}
\end{Highlighting}
\end{Shaded}
...
Il s'agit bien ici d'indexation par comparaison. La syntaxe est
similaire à celle que nous avons vue pour les vecteurs, mais cette fois,
l'objet sur lequel porte la condition (à l'intérieur des crochets) est
une colonne du \texttt{data.frame} (par exemple, \texttt{Bruant\$Sexe}
ou \texttt{Bruant\$Masse}), et l'objet dont on fournit le nom à gauche
des crochets est le \texttt{data.frame} lui-même (\texttt{Bruant}).
COmme pour l'indexation par position, tout ce qui se trouve à gauche de
la virgule entre les crochets correspond aux lignes, et tout ce qui se
trouve à droite de la virgule correspond aux colonnes. Ici, on omet
l'indice des colonnes pour afficher toutes les colonnes.

Pour expliquer concrètement ce qui se passe, prenons l'exemple de la
commande \texttt{Bruant{[}Bruant\$Masse\ \textgreater{}\ 15,\ {]}}. La
partie \texttt{Bruant\$Masse\ \textgreater{}\ 15} produit un vecteur
logique (de longueur 8, comme le nombre de lignes du \texttt{data.frame}
\texttt{Bruant}) contenant des valeurs \texttt{TRUE} et \texttt{FALSE}
selon que la masse de chaque individu est supérieure à 15 g ou non. Ce
vecteur logique est ensuite utilisé pour sélectionner les lignes du
\texttt{data.frame} \texttt{Bruant} car on place cette commande entre
crochets, mais à gauche de la virgule : seules les lignes pour
lesquelles la condition est \texttt{TRUE} sont affichées, les autres
sont ignorées :

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{Bruant}\SpecialCharTok{$}\NormalTok{Masse }\SpecialCharTok{\textgreater{}} \DecValTok{15}
\end{Highlighting}
\end{Shaded}
...
Jusqu'ici, nous n'avons qu'affiché le résultat des commandes dans la
console. Si on souhaite pouvoir réutiliser ces données filtrées plus
tard, il faut les stocker dans un nouvel objet. Par exemple, pour créer
un nouveau \texttt{data.frame} nommé \texttt{Bruant\_lourd} contenant
uniquement les individus dont la masse est supérieure à 15 g, on tape :

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{Bruant\_lourd }\OtherTok{\textless{}{-}}\NormalTok{ Bruant[Bruant}\SpecialCharTok{$}\NormalTok{Masse }\SpecialCharTok{\textgreater{}} \DecValTok{15}\NormalTok{, ]}
\end{Highlighting}
\end{Shaded}

On peut maintenant travailer avec ce nouvel objet et le anipuler comme
n'importe quel tableau de données :

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Affichage du nouveau tableau}
\NormalTok{Bruant\_lourd}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Affichage de ses dimensions}
\FunctionTok{dim}\NormalTok{(Bruant\_lourd)}
\end{Highlighting}
\end{Shaded}
...
Pour finir sur ces questions d'indexation (mais nous y reviendront plus
en détail dans le Chapitre~\ref{sec-wrangling}, à l'aide de packages
spécifiques qui rendent les choses plus simples), sachez que ces
notations avec des crochets, des dollars, des indices positifs ou
négatifs sont souvent difficiles à maitriser pour les débutants. C'est
normal : il s'agit de notation peu parlantes et peu intuitives. Il est
toutefois important de les comprendre, car il s'agit de la méthode
d'indexation historique, toujours très utilisées dans \texttt{R} (ainsi
que sur les forums d'entraide sur internet et par les IA génératives).
Être capables de déchiffrer, comprendre et manipuler ces notations sera
donc important pour la suite.

Toutefois, il existe une alternative plus ``moderne'' et intuitive à ces
notations, qui repose sur l'utilisation de fonctions spécifiques,
notamment grâce au package \texttt{dplyr}. Vous en verrez plusieurs
exemples détaillés dans le Chapitre~\ref{sec-wrangling}. Dans
l'immédiat, voilà un exemple d'utilisation de la fonction
\texttt{subset()} (du package \texttt{base}), qui permet de faire des
sous-ensembles de tableaux de données sans recourrir aux notations
alambiquées vues plus haut. Par exemple, pour créer le même tableau
\texttt{Bruant\_lourd} que précédemment, on peut taper :

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{Bruant\_lourd }\OtherTok{\textless{}{-}} \FunctionTok{subset}\NormalTok{(Bruant, Masse }\SpecialCharTok{\textgreater{}} \DecValTok{15}\NormalTok{)}
\NormalTok{Bruant\_lourd}
\end{Highlighting}
\end{Shaded}
...
De même, pour créer un tableau \texttt{Bruant\_femelle} contenant
uniquement les individus de sexe ``Fe'', on peut taper :

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{Bruant\_femelle }\OtherTok{\textless{}{-}} \FunctionTok{subset}\NormalTok{(Bruant, Sexe }\SpecialCharTok{==} \StringTok{"Fe"}\NormalTok{)}
\NormalTok{Bruant\_femelle}
\end{Highlighting}
\end{Shaded}
...
Vous remarquez qu'il n'y ap lus de crochets, il n'y a plus non plus de
dollar, et qu'il n'y a plus besoin de préciser de quel objet proviennent
les variables \texttt{Masse} et \texttt{Sexe}.

\subsection{Opérateurs de comparaison}\label{sec-comparaison}
...
\begin{tcolorbox}[enhanced jigsaw, coltitle=black, toprule=.15mm, opacityback=0, breakable, colback=white, opacitybacktitle=0.6, colframe=quarto-callout-warning-color-frame, leftrule=.75mm, rightrule=.15mm, titlerule=0mm, bottomtitle=1mm, toptitle=1mm, arc=.35mm, title=\textcolor{quarto-callout-warning-color}{\faExclamationTriangle}\hspace{0.5em}{Erreur ou avertissement ? \faIcon{bomb} ou \faIcon{exclamation-triangle}
?}, colbacktitle=quarto-callout-warning-color!10!white, bottomrule=.15mm, left=2mm]
...
\subsection{L'utilisation des fonctions}\label{sec-functions}
...
\begin{longtable}[]{@{}
  >{\raggedleft\arraybackslash}p{(\linewidth - 2\tabcolsep) * \real{0.2800}}
...
\begin{tcolorbox}[enhanced jigsaw, coltitle=black, toprule=.15mm, opacityback=0, breakable, colback=white, opacitybacktitle=0.6, colframe=quarto-callout-note-color-frame, leftrule=.75mm, rightrule=.15mm, titlerule=0mm, bottomtitle=1mm, toptitle=1mm, arc=.35mm, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Note : \texttt{install.packages()}}, colbacktitle=quarto-callout-note-color!10!white, bottomrule=.15mm, left=2mm]
...
\begin{tcolorbox}[enhanced jigsaw, coltitle=black, toprule=.15mm, opacityback=0, breakable, colback=white, opacitybacktitle=0.6, colframe=quarto-callout-note-color-frame, leftrule=.75mm, rightrule=.15mm, titlerule=0mm, bottomtitle=1mm, toptitle=1mm, arc=.35mm, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Note : \texttt{library()}}, colbacktitle=quarto-callout-note-color!10!white, bottomrule=.15mm, left=2mm]
...
\pandocbounded{\includegraphics[keepaspectratio]{images/Penguins.png}}
...
\begin{tcolorbox}[enhanced jigsaw, coltitle=black, toprule=.15mm, opacityback=0, breakable, colback=white, opacitybacktitle=0.6, colframe=quarto-callout-tip-color-frame, leftrule=.75mm, rightrule=.15mm, titlerule=0mm, bottomtitle=1mm, toptitle=1mm, arc=.35mm, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{Quizz : à quoi correspondent chacune des lignes de ce tableau ?}, colbacktitle=quarto-callout-tip-color!10!white, bottomrule=.15mm, left=2mm]
...
avec un écart-type de 801.95 grammes (colonne \texttt{sd}), et que la
...
conseille de garder le @\#sec-basics à portée de main afin de pouvoir
refaire les parties que vous maîtrisez le moins. Ce n'est qu'en relisant
plusieurs fois ces tutoriels que les choses seront vraiment comprises et
que vous les retiendrez. Ainsi, si des éléments de code présentés
ci-dessous vous semblent obscurs, revenez en arrière : toutes les
réponses à vos questions se trouvent probablement dans les chapitres
précédents.
...
\begin{tcolorbox}[enhanced jigsaw, coltitle=black, toprule=.15mm, opacityback=0, breakable, colback=white, opacitybacktitle=0.6, colframe=quarto-callout-tip-color-frame, leftrule=.75mm, rightrule=.15mm, titlerule=0mm, bottomtitle=1mm, toptitle=1mm, arc=.35mm, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{Objectifs}, colbacktitle=quarto-callout-tip-color!10!white, bottomrule=.15mm, left=2mm]
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/fig-gapminder-1.pdf}}
...
2026) permet de réaliser des graphiques dans \texttt{R} en respectant
...
\includegraphics[width=0.5\linewidth,height=\textheight,keepaspectratio]{images/culmen_depth.png}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-4-1.pdf}}
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-5-1.pdf}}
...
\begin{tcolorbox}[enhanced jigsaw, coltitle=black, toprule=.15mm, opacityback=0, breakable, colback=white, opacitybacktitle=0.6, colframe=quarto-callout-tip-color-frame, leftrule=.75mm, rightrule=.15mm, titlerule=0mm, bottomtitle=1mm, toptitle=1mm, arc=.35mm, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{En résumé}, colbacktitle=quarto-callout-tip-color!10!white, bottomrule=.15mm, left=2mm]
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-7-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-9-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-10-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-11-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-12-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-13-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-14-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-15-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-16-1.pdf}}
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-17-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-19-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-20-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-21-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-22-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-24-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-25-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/fig-cloud-1.pdf}}
...
\includegraphics[width=0.5\linewidth,height=\textheight,keepaspectratio]{images/pch.png}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-26-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-27-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/fig-jitter-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/fig-jitter-2.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/fig-jitter-3.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/fig-strip-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/fig-barplot-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/fig-barplotcol-1.pdf}}
...
\begin{tcolorbox}[enhanced jigsaw, coltitle=black, toprule=.15mm, opacityback=0, breakable, colback=white, opacitybacktitle=0.6, colframe=quarto-callout-important-color-frame, leftrule=.75mm, rightrule=.15mm, titlerule=0mm, bottomtitle=1mm, toptitle=1mm, arc=.35mm, title=\textcolor{quarto-callout-important-color}{\faExclamation}\hspace{0.5em}{Diagrammes bâtons}, colbacktitle=quarto-callout-important-color!10!white, bottomrule=.15mm, left=2mm]
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/fig-bpspecies-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/fig-bpspecies-infreq-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-34-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-35-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-36-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-36-2.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/fig-pie-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/fig-bpspecies-sex-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-37-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-38-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-39-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-41-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-42-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-43-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{images/map.png}} En
règle général, les cartes portent un grand nombre de variables,
numériques et/ou catégorielles. Mais tout commence toujours par 2
variables numériques, les latitudes et longitude des
structures/informations que l'on souhaite représenter (traits de côte,
profiles bathymétriques, lieux d'observations diverses, \ldots).
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-44-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-45-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-46-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-47-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-48-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-49-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-50-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-51-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-52-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-53-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-54-1.pdf}}
...
\texttt{ggmosaic} de Jeppson, Hofmann, et Cook (2026) est toutefois
entièrement dédié à ce type de graphique. Pour installer ce packages, il
faut procéder un peu différemment de d'habitude car nous devons
installer sa version de développement (c'est une version plus récente
que celle qui est disponible sur les serveurs officiels du CRAN). Pour
cela, nous allons utiliser le package \texttt{remotes}. Si vous ne
l'avez pas encore installé, faites-le maintenant :
...
Attention, il est possible que \texttt{R} vous propose d'installer
d'autres packages dont \texttt{remotes} dépend. Si c'est le cas, vous
devrez l'indiquer dans la console quand \texttt{R} vous posera la
question en tapant la réponse appropriée (par exemple, le chiffre
\texttt{1} pour installer toutes les dependances conseillées). Il est
également possible que l'instalation de certaines de ces dependances ne
soit pas possible tant que \texttt{R} n'aura pas été redémarré. Si c'est
le cas, redémarrez \texttt{R} (en cliquant dans le menu
\texttt{Session\ \textgreater{}\ Restart\ R} dans RStudio). puis
relancez la commande \texttt{install.packages("remotes")} jusqu'à ce que
l'installation se fasse sans erreur. Vous pourrez alors installer la
version de développement du package \texttt{ggmosaic} ainsi :

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{remotes}\SpecialCharTok{::}\FunctionTok{install\_github}\NormalTok{(}\StringTok{"haleyjeppson/ggmosaic"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

Si vous avez dû redémarrer \texttt{R} pour installer le package
\texttt{remotes}, n'oubliez pas de recharger l'ensemble des packages
nécessaires pour ce chapitre :
...
Ajoutez-y le package \texttt{ggmosaic} nouvellement installé :
...
Vous pouvez maintenant accéder à un nouvel objet géométrique,
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-59-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-60-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-61-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/fig-facstrip-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-62-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-63-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-64-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/fig-comboxplot-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-65-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-66-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-67-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-68-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-69-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/fig-multivar-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-70-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-71-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-72-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-73-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-74-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-76-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-77-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/fig-label-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-78-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-79-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-80-1.pdf}}

Voici toutes les possibilités :

\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-81-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-82-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-83-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-84-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-85-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-86-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-87-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{images/brewer.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-88-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-89-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-90-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-91-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-92-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-93-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-94-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-95-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-96-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-97-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-98-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{images/fonts1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{images/fonts2.pdf}}
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-104-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-106-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-visualization_files/figure-pdf/unnamed-chunk-107-1.pdf}}
...
{\centering \includegraphics[width=0.7\linewidth,height=\textheight,keepaspectratio]{images/import.png}
...
{\centering \includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{images/import2.png}
...
{\centering \includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{images/import3.png}
...
{\centering \includegraphics[width=0.8\linewidth,height=\textheight,keepaspectratio]{images/importcsv1.png}
...
{\centering \includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{images/importcsv2.png}
...
{\centering \includegraphics[width=0.5\linewidth,height=\textheight,keepaspectratio]{images/importcsv3.png}
...
{\centering \includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{images/importcsv4.png}
...
\pandocbounded{\includegraphics[keepaspectratio]{04-DataWrangling_files/figure-pdf/unnamed-chunk-11-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{04-DataWrangling_files/figure-pdf/unnamed-chunk-13-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{04-DataWrangling_files/figure-pdf/unnamed-chunk-14-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{04-DataWrangling_files/figure-pdf/unnamed-chunk-15-1.pdf}}
...
{\centering \includegraphics[width=0.5\linewidth,height=\textheight,keepaspectratio]{images/filter.png}
...
{\centering \includegraphics[width=0.5\linewidth,height=\textheight,keepaspectratio]{images/select.png}
...
{\centering \includegraphics[width=0.5\linewidth,height=\textheight,keepaspectratio]{images/mutate.png}
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{04-DataWrangling_files/figure-pdf/consommation-1.pdf}}
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{04-DataWrangling_files/figure-pdf/consommation2-1.pdf}}
...
{\centering \includegraphics[width=0.4\linewidth,height=\textheight,keepaspectratio]{images/arrange.png}
...
Jeppson, Haley, Heike Hofmann, et Di Cook. 2026. \emph{ggmosaic: Mosaic
...
Dunnington, et Teun van den Brand. 2026. \emph{ggplot2: Create Elegant
